...
@endsection
@section('content')
    <div class="container mt-5" id="Glass" style="margin-bottom: 100px">
        <div :class="message ? 'alert alert-primary':''">
            @{{ message }}
        </div>
        <div class="text-center d-flex flex-column align-items-center">
            <h3 class="mb-2" style="font-family: 'Unbounded'; font-weight:400">Админ.Стеклопакеты</h3>
            ...
            <table class="table">
                ...
                <tbody>
                  <tr v-for="category in categories">
                    ...
                    <td><img :src="'/public/'+category.img" alt=""></td>
                    <td>
                        <div><a class="btn btn-danger" :href="`{{ route('deleteGlass') }}/${category.id}`">Удалить</a></div>
                        <div>
                            <button type="button" style="background: #4F8BD2; color:white" class="btn" data-bs-toggle="modal" :data-bs-target="'#exampleModal'+category.id">
                                Редактировать
                            </button>
                            <div class="modal fade" :id="'exampleModal'+category.id" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form :id="'formUpdate'+category.id" @submit.prevent="update(category.id)">
                                            <div class="mb-3">
                                                <label for="title" style="color: black" class="form-label">Название</label>
                                                <input :value="category.title" type="text" class="form-control" style="border-radius: 0; border:1px solid black" id="title" name="title" :class="errors.title ? 'is-invalid':''">
                                                <div :class="errors.title ? 'invalid-feedback':''" v-for="error in errors.title">
                                                  @{{ error }}
                                                </div>
                                              </div>
                                              <div class="mb-3">
                                                  <label for="text" style="color: black" class="form-label">Описание</label>
                                                  <textarea type="text" class="form-control" style="border-radius: 0; border:1px solid black" id="text" name="text" :class="errors.text ? 'is-invalid':''">@{{ category.text }}</textarea>
                                                  <div :class="errors.text ? 'invalid-feedback':''" v-for="error in errors.text">
                                                    @{{ error }}
                                                  </div>
                                              </div>
                                            ...
                                            <button type="submit" style="background: #4F8BD2; color:white" class="btn">Сохранить</button>
                                        </form>
                                    </div>
                                </div>
                                </div>
        ...
        const app = {
            ...
            methods: {
                ...
                async add(){
                    ...
                    const response = await fetch('{{ route('addGlass') }}',{
                        method:'post',
                        headers:{
                ...
                async update(id){
                    ...
                    const response = await fetch('{{ route('updateGlass') }}',{
                        method:'post',
                        headers:{
                            'X-CSRF-TOKEN':'{{ csrf_token() }}'
                        },
